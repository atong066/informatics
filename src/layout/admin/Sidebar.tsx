import { FiBookOpen, FiGrid, FiLayers, FiLogOut, FiShield, FiUserCheck, FiUsers, FiX } from 'react-icons/fi';
...
const adminTools = [
  ...
  { label: 'HR Accounts', icon: FiUserCheck, path: '/admin/hr' },
];
...
function AdminSidebar({
  ...
  return (
    <aside
      className={`h-[100dvh] max-h-[100dvh] w-[80vw] max-w-[288px] overflow-hidden border-r border-white/10 bg-[linear-gradient(180deg,#17333b_0%,#1f4652_55%,#295766_100%)] px-3 py-4 pb-[max(0.16rem,env(safe-area-inset-bottom))] text-white shadow-[0_20px_40px_rgba(9,31,37,0.3)] ${
        isOpen ? 'translate-x-0' : '-translate-x-full'
      } transition-transform duration-300 ease-out lg:h-full lg:max-h-none lg:w-full lg:max-w-none lg:translate-x-0 lg:shadow-none`}
      onClick={(event) => event.stopPropagation()}
    >
      <div className="flex h-full min-h-0 flex-col">
        <div className="flex items-center justify-between gap-3 px-2">
          <div className="flex items-center gap-3">
            <div className="flex h-11 w-11 items-center justify-center rounded-[0.16rem] bg-[linear-gradient(180deg,#2aa493_0%,#1b7d71_100%)] shadow-[0_12px_22px_rgba(9,31,37,0.22)]">
              <img
                src="/images/logo.png"
          ...
          <button
            type="button"
            onClick={() => onClose?.()}
            className="flex h-9 w-9 items-center justify-center rounded-[0.16rem] border border-white/10 bg-white/8 text-[#d8eff1] lg:hidden"
        ...
        <div className="scrollbar-super-thin mt-5 min-h-0 flex-1 overflow-y-auto overscroll-contain pr-1">
          ...
          <div className="mt-5 px-2 pb-4">
            ...
            <div className="mt-3 rounded-[0.192rem] border border-white/10 bg-white/8 px-4 py-4 text-[#d7eaee]">
              <div className="flex items-center gap-3">
