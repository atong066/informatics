      ...
      <div className="mx-auto w-full max-w-[90rem] px-4 py-5 sm:px-6 lg:px-8">
        <section className="rounded-[1.65rem] border border-[#b2c3d1] bg-[linear-gradient(180deg,rgba(212,222,233,0.97)_0%,rgba(201,212,225,0.95)_100%)] px-5 py-5 shadow-[0_10px_22px_rgba(27,46,70,0.08)]">
          {submissionsQuery.isLoading ? (
            <p className="text-[0.95rem] text-[#6b8198]">Loading activity submissions...</p>
          ) : payload ? (
            <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
              ...
              <div className="grid min-w-[15rem] gap-3 rounded-[1.2rem] bg-[linear-gradient(180deg,#365678_0%,#2d4868_100%)] px-4 py-4 text-white shadow-[0_10px_20px_rgba(27,46,70,0.12)]">
                <div className="flex items-center gap-2 text-[#d5e2ef]">
                  ...
                  <p className="text-[0.76rem] uppercase tracking-[0.16em]">Submission summary</p>
                </div>
            ...
            <div className="mt-5">
              {payload.submittedStudents.length > 0 ? (
                  ...
                  {payload.submittedStudents.map((student) => (
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          <h2 className="truncate text-[1rem] font-semibold text-[#173b70]">
                            {student.fullName}
                          </h2>
                          <p className="mt-1 text-[0.84rem] text-[#5e7891]">
                            @{student.username} • {student.section}
                          </p>
                        </div>
                        <span className="shrink-0 rounded-full border border-[#bce8cf] bg-[#effbf4] px-3 py-1 text-[0.72rem] font-semibold text-[#12815a]">
                          Submitted
                        </span>
                      </div>
                      ...
                      <div className="mt-4 grid gap-3 sm:grid-cols-2">
                        <div className="rounded-[1rem] border border-[#bccbd7] bg-[linear-gradient(180deg,#dbe5ed_0%,#d0dbe5_100%)] p-4">
                          ...
                          <p className="mt-3 break-all text-[0.84rem] text-[#173b70]">{student.email}</p>
                        </div>
                      ...
                      {student.textContent ? (
                        <div className="mt-4 rounded-[1rem] border border-[#bccbd7] bg-[linear-gradient(180deg,#dbe5ed_0%,#d0dbe5_100%)] p-4">
                          ...
                          <p className="mt-3 text-[0.84rem] leading-[1.65] text-[#617d98]">
                            {student.textContent}
                          </p>
                        </div>
                      ) : null}
                        ...
                        <div className="mt-4 space-y-2">
                          {student.attachments.map((attachment) => (
                            <a
                              ...
                              className="flex items-center justify-between gap-3 rounded-[1rem] border border-[#c3d2de] bg-[rgba(214,224,234,0.94)] px-4 py-3 text-[0.82rem] font-medium text-[#2f78bc] transition hover:bg-[rgba(221,230,238,0.98)] hover:text-[#215f99]"
                            >
                ...
                <div className="rounded-[1.4rem] border border-dashed border-[#b2c2d0] bg-[linear-gradient(180deg,#d2dde8_0%,#c7d4e0_100%)] px-6 py-10 text-center">
                  <p className="text-[0.98rem] font-semibold text-[#173b70]">No submissions yet</p>
